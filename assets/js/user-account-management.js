...
function checkIfAccountExists() {
  ...
  var waitTime = 500; // Wait time in milliseconds

  var restRoot = user_account_management_rest.site_url + user_account_management_rest.rest_namespace; // Listen for `keyup` event
  ...
  var search = function search(text) {
    fetch(restRoot + '/check-account/?email=' + text, {
      ...
      if ('success' === data.status && 'user exists' === data.reason) {
        //console.log('success');
        var emailExists = document.createElement('div');
        emailExists.innerHTML = '<p class="a-form-caption a-account-exists a-account-exists-success">This email address already has a MinnPost.com account. You can <a href="' + user_account_management_rest.site_url + '/user/login/?email=' + text + '">log in</a> or <a href="' + user_account_management_rest.site_url + '/user/password-lost/?email=' + text + '">reset your password</a>.</p>';
