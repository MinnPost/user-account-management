...
function passwordStrengthChecker(password, strengthMeter, strengthText, submitButton, disallowListArray) {
  ...
  const strength = wp.passwordStrength.meter(password, disallowListArray, password);
  switch (strength) {
    case 2:
...
function setupPasswordStrength() {
  const checkPasswordStrength = document.querySelector('.password-strength-check');
  if (checkPasswordStrength) {
    const beforePasswordChecker = document.querySelector('.a-form-show-password');
...
function checkZipCountry(cityField, stateField, zipField, countryField) {
  ...
  if ('' !== zipValue) {
    ...
    if (locationElement) {
      let url = user_account_management_rest.site_url + user_account_management_rest.rest_namespace + '/check-zip';
